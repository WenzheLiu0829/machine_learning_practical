...
class AdaGradLearningRule(GradientDescentLearningRule):
    ...
    def initialise(self, params):
        ...
        for param in self.params:
            self.Sum_squared_grads.append(np.zeros_like(param))          
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, Sum_squared_grad, grad in zip(self.params, self.Sum_squared_grads, grads_wrt_params):
            
            Sum_squared_grad += grad**2
            AdaGrad = - (self.learning_rate * grad) / ((np.sqrt(Sum_squared_grad) + self.Epsilon))
            param += AdaGrad
...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, Epsilon = 1e-8, Decay_rate = 0.9):
        """Creates a new learning rule object.
    ...
    def initialise(self, params):
        ...
        self.Average_squared_grads = []
        for param in self.params:
            self.Average_squared_grads.append(np.zeros_like(param))  
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, Average_squared_grad, grad in zip(self.params, self.Average_squared_grads, grads_wrt_params):
            Average_squared_grad *= self.Decay_rate
            Average_squared_grad += np.square(grad) * (1 - self.Decay_rate)
            RMSProp = - (self.learning_rate * grad) / ((np.sqrt(Average_squared_grad) + self.Epsilon))
            param += RMSProp